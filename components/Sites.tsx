...
const SiteMap = dynamic(() => import("./SiteMap"), {
  ssr: false,
  loading: () => <div className="h-full w-full animate-pulse bg-navy/10" />,
});
...
export function Sites() {
  ...
  return (
    <section id="sites" className="bg-navy/[0.03] py-16 md:py-24">
      <div className="mx-auto max-w-content px-5">
        <Reveal className="max-w-2xl">
          <span className="text-[12px] font-semibold uppercase tracking-wide text-orange-text">{t.sites.eyebrow}</span>
          <h2 className="mt-2 font-display text-2xl font-bold text-navy md:text-3xl">{t.sites.title}</h2>
          <p className="mt-3 text-[14px] text-navy-deep/70">{t.sites.subtitle}</p>
        </Reveal>
        ...
        <div className="mt-10 grid gap-4 lg:grid-cols-[minmax(0,1fr)_1.3fr] lg:items-stretch">
          ...
          <div
            ref={mapWrapRef}
            className="order-1 h-[320px] overflow-hidden rounded-2xl shadow-sm ring-1 ring-black/5 lg:order-2 lg:h-auto lg:min-h-[420px]"
          >
            {mapNearView ? (
